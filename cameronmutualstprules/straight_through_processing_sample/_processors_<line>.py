...
@dynamic_defaults
def rule_test_something_v1(param="{bc::<Group>::<Section>::<Value of question>}}", fallback=True):
    if param:
...
def rule_test_something_else_v1(entire_data_object):
...
@dynamic_defaults
def rule_test_another_thing_v1(param="{bc::<Revision/Policy/Item etc>::Section::<Value>}}", fallback=True):
    if param:
        success_value = param == "value"
        message = "something you would want to return if it's not successful"
        return_message = message if not success_value else None
